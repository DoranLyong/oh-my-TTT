...
from utils_ema import load_checkpoint, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor, load_pretrained
...
def main():
    ...
    args.model_ema_decay = args.model_ema_decay ** (effective_batch_size / 4096.0)
    ...
    if args.model_ema:
        ...
        if config.MODEL.RESUME:
            ...
            if 'state_dict_ema' in checkpoint:
                ema_model = unwrap_model(model_ema.ema)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, total_epochs):
        ...
        train_one_epoch(config, model, model_ema, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, logger, total_epochs, mesa=config.TRAIN.MESA if epoch >= int(0.25 * total_epochs) else -1.0, grad_accum_steps=grad_accum_steps)
        acc1, acc5, loss = validate(config, data_loader_val, model, logger)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        ...
        if dist.get_rank() == 0 and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs)):
            save_checkpoint(config, epoch + 1, model_without_ddp, model_ema, max(max_accuracy, acc1), max(max_accuracy_e, acc1_e), optimizer, lr_scheduler, logger)

        if dist.get_rank() == 0 and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs)) and acc1 >= max_accuracy:
            save_checkpoint(config, epoch + 1, model_without_ddp, model_ema, max(max_accuracy, acc1), max(max_accuracy_e, acc1_e), optimizer, lr_scheduler, logger, name='max_acc')
        max_accuracy = max(max_accuracy, acc1)
        logger.info(f'Max accuracy: {max_accuracy:.2f}%')

        if model_ema is not None and not args.model_ema_force_cpu:
            if dist.get_rank() == 0 and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs)) and acc1_e >= max_accuracy_e:
                save_checkpoint(config, epoch + 1, model_without_ddp, model_ema, max(max_accuracy, acc1), max(max_accuracy_e, acc1_e), optimizer, lr_scheduler, logger, name='max_ema_acc')
...
def train_one_epoch(config, model, model_ema, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, logger, total_epochs, mesa=1.0, grad_accum_steps=1):
    model.train()
